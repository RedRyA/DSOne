...
/*
 * Provided code - DO NOT CHANGE THIS METHOD OTHER THAN TO ADD YOUR NAME AND
 * YOUR PARTNER'S NAME (if you make alterations for the sake of testing be sure
 * to revert them before submission)
 */
int main(int argc, char *argv[]) {
  functionRuntimes fRT;

  int sizes1[] = {500,   1000,  2000,  4000,   8000,
  ...
  fRT =
      timeAlgorithm("quicksort (uses insertion sort when sorting <30 numbers)",
                    12, 8, sizes1, quickSortOptInitial);
  printRuntimeTable(fRT);
  freeFunctionRuntimes(fRT);
...
/*************************************** Functions for finding and printing
 * runtime *********************************************/

/*
TODO: Give your asymptotic estimates for the runtimes of the following 3
functions:
 ...
 */
functionRuntimes timeAlgorithm(char *szName, int iNumRepeats,
                               int iNumTestCaseSizes, int arrTestSizes[],
                               void (*f)(FILE *)) {
  ...
  // TODO: copy passed data into the fRT variable.  Specifically do the
  // following:
  ...
  /*DONE  malloc space for arrTestSizes in fRT to hold iNumTestCaseSizes number
   * of ints */
  fRT.arrTestSizes = (int *)malloc(sizeof(int) * iNumTestCaseSizes);
  /*DONE  fill arrTestSizes in fRT with the variable arrTestSizes (hint: use a
   * loop) */
  ...
  for (i = 0; i < iNumTestCaseSizes; i++) {
    ...
    for (j = 0; j < iNumRepeats; j++) {
      ...
      // TODO: uncomment the next line line after you've malloc-ed memory for
      // fRT.arrRuntimes
      fRT.arrRuntimes[i][j] = (double)(end - start) / CLOCKS_PER_SEC;
    }
  }

  // TODO: on the next line, malloc space for fRT.arrAvg (you'll need to store
  // iNumTestCaseSizes variables of type double)
  fRT.arrAvg = (double *)malloc(
...
/*for(i =0; i<fRT.arrAvg;i++){ 
avgCount=fRT.arrAvg[i]-fRT.arrAvg[i-1];
if((i-1)<0) {
	
	printf("N/A");
}
//printf("Increase %lf",avgCount);
printf("ARRAY AVG TEST%d",i);
}*/
